...
def dfFromGameLogURLList(gamelogs, dataframes=None, regular=True):
    """
    ...
    """
    if dataframes is None and regular is True:
        dataframes = [regular_dfFromGameLogURL(g) for g in gamelogs]
    elif dataframes is None and regular is False:
        dataframes = [playoff_dfFromGameLogURL(g) for g in gamelogs]
...
def regular_dfFromGameLogURL(url):
    """
    Takes a url of a player's game log for a given year, returns a regular season DataFrame
    """
    ...
    if reg is None:
        print("Not found regular season dataframes")
        return None
...
def find_pbp_table(glsoup):
    pbp_table = glsoup.find_all('table', id="pbp")  # id for playoff table
    if len(pbp_table) > 0:
        return pbp_table
    div_soup = glsoup.find("div", id="all_pbp")
    if div_soup is None:
        return []
    pbp_soup = find_html_in_comment(div_soup)
    if pbp_soup is None:
        return []
    pbp_table = pbp_soup.find_all('table', id="pbp")
    return pbp_table
...
def allRegualrGameLogs(playerDictionary, name, dataframes=None):
    # would be nice to put some caching logic here...
    return dfFromGameLogURLList(playerDictionary.get(name).gamelog_url_list, dataframes, True)


def allPlayoffGameLogs(playerDictionary, name, dataframes=None):
    return dfFromGameLogURLList(playerDictionary.get(name).gamelog_url_list, dataframes, False)


def regularSeasonGameLogs(playerDictionary, name, season):
    return regular_dfFromGameLogURL(playerDictionary.get(name).gamelog_url_dict.get(season))


def playoffSeasonGameLogs(playerDictionary, name, season):
    return playoff_dfFromGameLogURL(playerDictionary.get(name).gamelog_url_dict.get(season))


def play_by_playLogs(playerDictionary, name):
    return pbp_dfFromGameLogURL(playerDictionary.get(name).overview_url)
